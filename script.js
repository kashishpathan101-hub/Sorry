...
function goToSlide(index) {
  currentSlide = index;
  slidesWrapper.style.transform = `translateX(-${index * 100}%)`; /* Yaha 100% kiya */
...
function c1MoveNo() {
  ...
  btn.style.transform = `translate(${x}px, ${y}px)`; /* backticks lagaye */
}
...
function cyclePhotos() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      firstCard.style.transition = "transform 0.45s ease, opacity 0.35s ease"; /* space add kiya */
    }, 50);
  }, 350);
...
function initBowGame() {
  ...
  function drawHeart(x, y, size) {
    ...
    ctx.bezierCurveTo(-size, size * 0.2, -size, -size * 0.6, 0, -size * 0.15);
    ctx.bezierCurveTo(size, -size * 0.6, size, size * 0.2, 0, size * 0.8);
    ctx.fill();
    ctx.restore();
  }
  ...
  function drawBow() {
    ...
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.arc(bow.x, bow.y, 42, Math.PI * 0.15, Math.PI * 0.85);
    ctx.stroke();
  ...
  function releaseArrow() {
    ...
    arrow.x = bow.x;
    arrow.y = bow.y;
    arrow.vy = -Math.max(pull * 0.45, 8);
    arrow.vx = (bow.x - drag.x) * 0.25;
  }
